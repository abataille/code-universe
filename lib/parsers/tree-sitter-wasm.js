...
const FILE_GRAMMARS = new Map([
  ...
  [".py", { language: "python", packageName: "tree-sitter-python", wasmFile: "tree-sitter-python.wasm" }],
  [".pyw", { language: "python", packageName: "tree-sitter-python", wasmFile: "tree-sitter-python.wasm" }],
  [".php", { language: "php", packageName: "tree-sitter-php", wasmFile: "tree-sitter-php.wasm" }],
  [".phtml", { language: "php", packageName: "tree-sitter-php", wasmFile: "tree-sitter-php.wasm" }],
  [".java", { language: "java", packageName: "tree-sitter-java", wasmFile: "tree-sitter-java.wasm" }]
]);
  ...
  while (stack.length) {
    ...
    if (node.isNamed && (node === root || INTERESTING_NODE_TYPES.has(node.type))) {
      interestingNodes.push({
        type: node.type,
        name: syntaxNodeName(node),
        range: rangeForNode(node, base),
        named: true
      });
    }
...
function syntaxNodeName(node) {
  const declarationChild = node.childForFieldName?.("declarator")
    || node.namedChildren.find((child) => ["variable_declarator", "variable_name"].includes(child.type));
  const namedChild = node.childForFieldName?.("name")
    || declarationChild?.childForFieldName?.("name")
    || node.namedChildren.find((child) => [
      "identifier", "type_identifier", "property_identifier", "namespace_name", "qualified_name", "variable_name"
    ].includes(child.type));
