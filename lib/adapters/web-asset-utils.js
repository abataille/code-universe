...
async function readImageDimensions(absolute) {
  ...
  if (buffer.length >= 26 && buffer.subarray(0, 2).toString("hex") === "424d") {
    return {
      pixelWidth: Math.abs(buffer.readInt32LE(18)),
      pixelHeight: Math.abs(buffer.readInt32LE(22))
    };
  }
  if (buffer.length >= 8 && buffer.readUInt16LE(0) === 0 && buffer.readUInt16LE(2) === 1 && buffer.readUInt16LE(4) > 0) {
    return {
      pixelWidth: buffer[6] || 256,
      pixelHeight: buffer[7] || 256
    };
  }
  if (buffer.length >= 24 && ["avif", "avis"].includes(buffer.subarray(8, 12).toString())) {
    const marker = buffer.indexOf(Buffer.from("ispe"));
    if (marker >= 4 && marker + 12 <= buffer.length) {
      return { pixelWidth: buffer.readUInt32BE(marker + 4), pixelHeight: buffer.readUInt32BE(marker + 8) };
    }
  }
