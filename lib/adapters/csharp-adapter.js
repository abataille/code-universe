...
function scanFile(file, source, nodes, edges, declarations, pendingTypeEdges, pendingCalls) {
  ...
  const namespaceName = [...clean.matchAll(/\bnamespace\s+([A-Za-z_][\w.]*)\s*[;{]/g)][0]?.[1] || null;
  ...
  for (const match of clean.matchAll(TYPE_PATTERN)) {
    ...
    edges.push(edge(fileId, id, "defines", "csharp-syntax", 0.96));
    ...
    if (depth === 0 && trimmed) {
      ...
      if (method && !CONTROL_WORDS.has(method[2])) {
        ...
        const node = sourceNode(id, kind, name, file.relative, location.line, location.column, {
          qualifiedName,
          metrics: { parameters: parameterCount(method[3]) },
          attributes: {
        ...
        const location = lineColumn(source, offset + Math.max(0, line.indexOf(name)));
        const qualifiedName = `${type.qualifiedName}.${name}`;
        const id = `symbol:csharp:${file.relative}#${qualifiedName}:property:${location.line}`;
        const node = sourceNode(id, "property", name, file.relative, location.line, location.column, {
          qualifiedName,
          attributes: { type: property[1] }
        });
        addNode(nodes, node);
        declarations.push(node);
        edges.push(edge(type.id, id, "defines", "csharp-syntax", 0.92));
        pendingTypeEdges.push({ from: id, name: simpleTypeName(property[1]), kind: "uses" });
      }
    }
